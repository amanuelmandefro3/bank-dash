...
export const getAllCards = async (token: string) => {
  try {
    const response = await fetch(
      "https://bank-dash-36iy.onrender.com/cards?page=0&size=10",
      {
        method: "GET",
        headers: {
...
export const getCardById = async (id: string, token: string) => {
  try {
    const response = await fetch(`https://bank-dash-36iy.onrender.com/cards/${id}`, {
      method: "GET",
      headers: {
...
export const createCard = async (cardData: any, token: string) => {
  try {
    const response = await fetch("https://bank-dash-36iy.onrender.com/cards", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
